...
export function mergeBuyFileWithNextGen(
    ...
    for (const { items: groupItems, style } of groups.values()) {
        ...
        results.push({
            ...
            // NextGen now provides factory/cost — fallback to buy file values only if NextGen didn't fill them
            ...
            unitCost: ng?.unitCost ?? first.unitCost ?? null,
            costingReference: ng?.costingReference || first.costingReference || null,

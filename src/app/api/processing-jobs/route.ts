...
export async function POST(req: NextRequest) {
    ...
    void fetch(`${internalBase}/api/upload`, {
        method: "POST",
        headers: {
        ...
        if (!response.ok || result?.error) {
            updateProcessingJob(job.id, {
                status: "failed",
                error: result?.error || `Processing failed with HTTP ${response.status}`,
                result,
            });
            return;
        }
        ...
        updateProcessingJob(job.id, {
            status: "failed",
            error: error instanceof Error ? error.message : "Processing request failed",
        });
    });
